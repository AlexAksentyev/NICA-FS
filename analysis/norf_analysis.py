...
def analysis(path, eid, name='', axis=[1,0,0]):
    ...
    def plot_ps(dat):
        ...
        ax[0].plot(dat['X']*1e3, dat['A']*1e3, '.')
        ax[0].set_xlabel('X [mm]'); ax[0].set_ylabel('A [mrad]')
        ax[1].plot(dat['Y']*1e3, dat['B']*1e3, '.')
        ax[1].set_xlabel('Y [mm]'); ax[1].set_ylabel('B [mrad]')
        ax[2].plot(dat['T'], dat['D'], '.')
        ax[2].set_xlabel(r'$-(t-t_0)v_0\gamma/(1+\gamma)$ [m]'); ax[2].set_ylabel(r'$\Delta K/K$')
        for i in range(3):
            ax[i].ticklabel_format(axis='both', style='sci', scilimits=(0,0), useMathText=True)
            ax[i].grid()
        print('~~~~ OKAY ~~~~')
    ...
    ### making the PS plot
    print('ps shape', ps.data.shape)
    fps, axps = plot_ps(ps[:,1:150:50])
    plt.savefig(path+'img/ps_plot.png', dpi=450, bbox_inches='tight', pad_inches=.1)
    plt.close()
...
if __name__ == '__main__':
    common = HOMEDIR+'data/NO-RF/PROTON/300kTURN/'
    # main(common+'X-bunch/')
    # main(common+'Y-bunch/')
    main(common+'D-bunch/')
    common = HOMEDIR+'data/NO-RF/DEUTERON/300kTURN/'
    # main(common+'X-bunch/')
    # main(common+'Y-bunch/')
    # main(common+'D-bunch/')
